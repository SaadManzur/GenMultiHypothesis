...
def normalization_stats(complete_data, dim, predict_14=False ):
  ...
  if dim == 2:
    ...
    dimensions_to_ignore = np.delete( np.arange(len(H36M_NAMES)*3), dimensions_to_use ) # np.arange(len(H36M_NAMES)*3)
...
def transform_world_to_camera(poses_set, cams, ncams=4 ):
    ...
    for t3dk in sorted( poses_set.keys() ):
      ...
      for c in range( ncams ):
        ...
        camera_coord = np.reshape( camera_coord, [-1, len(H36M_NAMES)*3] ) #[-1, len(H36M_NAMES)*3]
...
def project_to_cameras( poses_set, cams, ncams=4 ):
  ...
  for t3dk in sorted( poses_set.keys() ):
    ...
    for cam in range( ncams ):
      ...
      pts2d = np.reshape( pts2d, [-1, len(H36M_NAMES)*2] ) #[-1, len(H36M_NAMES)*2]
      sname = seqname[:-3]+"."+name+".h5" # e.g.: Waiting 1.58860488.h5
